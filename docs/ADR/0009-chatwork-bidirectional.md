...
| 実行コンテキスト | `.\ejsan`（**対話セッション独立**・SIGINT 免疫・**2026-05-04 同日夜に LocalSystem から変更**・後述「H3 改訂」参照）|
...
##### H3 改訂: ObjectName を LocalSystem → `.\ejsan` に変更（2026-05-04 govcheck 同日夜）

H3 切替直後の govcheck で「Issue #30 監査ログが 2026-04-30 14:59 で更新停止」を検出。原因切り分け結果、**LocalSystem では IssuePoster (`gh issue edit` を spawn) が ejsan の GitHub credential keyring を読めず認証失敗**することが真因と判明（chatwork-mcp 自体は LocalSystem で動作可、`gh` CLI のみ ユーザーアカウント単位の keyring に依存）。

| 項目 | Before | After |
|---|---|---|
| ObjectName | `LocalSystem` | `.\ejsan` |
| `gh` CLI 認証 | ❌ keyring 読めず失敗 | ✅ ejsan の keyring 経由で成功 |
| Issue #30 audit_post | 🚨 4/30 以降停止 | ✅ 復活見込み（次回 audit 発火時に確認） |

###### 変更コマンド

```powershell
# password を Read-Host -AsSecureString で受取（履歴に残さない）
$secure = Read-Host -Prompt "ejsan local password" -AsSecureString
$BSTR = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($secure)
$pwd = [System.Runtime.InteropServices.Marshal]::PtrToStringAuto($BSTR)
ssh ejsan@100.104.151.97 ("C:\Tools\nssm\nssm.exe set AIEijiChatworkMCP ObjectName .\ejsan `"" + $pwd + "`"")
...
ObjectName が `.\ejsan` でも、サービス起動は **OS ログオン不要**（NTLM パスワードキャッシュベースの起動）。Autologon が切れても nssm がパスワード保持で起動できる。SIGINT 免疫・RDP 切断耐性は維持。
